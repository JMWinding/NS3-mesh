  ...
  do
    ...
    do
        ...
        if [ ! -f "${dirout}/mesh_${aa}_${bb}_${rr}.xmp" ]; then
          ...
          ./waf --run "mesh-loc-0-1 --gridSize=${aa} --apNum=${cc} --apStep=60 --aptx=true --gateways=1 --rateControl=${ratecontrol} --rndSeed=${rr} --totalTime=180 --flowout=${dirout}/mesh_${aa}_${bb}_${rr}.xmp --datarate=5e6" &> "${dirout}/mesh_${aa}_${bb}_${rr}.txt"
        fi
    done
  done
done
