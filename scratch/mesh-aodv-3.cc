...
AodvExample::CreateVariables ()
{
  ...
  lastTotalRx.reserve (apNum * clNum);
  throughput.reserve (apNum * clNum);
  packetSink.reserve (apNum * clNum);
      ...
      for (uint32_t i = 0; i < apNum; ++i)
        {
          ...
          for (uint32_t j = 0; j < clNum; ++j)
            {
         	  packetSink[i*clNum+j] = StaticCast<PacketSink> (clientApp[i].Get (j));
            }
