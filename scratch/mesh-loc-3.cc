  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if (i->second.rxPackets > 1)
        {
          std::cout << (double)i->second.rxBytes * 8/1024 / (i->second.timeLastRxPacket.GetSeconds () - i->second.timeFirstRxPacket.GetSeconds ()) << '\t';
        }
      else
        std::cout << 0 << '\t';
...
AodvExample::InstallMeshInternetStack ()
{
  ...
  if (printRoutes)
    {
//      Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("mesh-routing-"+std::string(route)+"/routes."+std::to_string (apNum)+"."+std::to_string (uint32_t (apStep))+"."+std::to_string (rndSeed), std::ios::out);
