  ...
  do
    ...
    do
      if [ ! -f "${dirout}grid_400_0_${aa}_${gg}_${rr}.txt" ]; then
        ...
        ./waf --run "mesh-loc-1 --apNum=${aa} --clNum=0 --aptx=true --rndSeed=${rr} -totalTime=40 --locationFile=${dirin}grid_400_0_${aa}.txt --scale=80 --gateways=${gg} --route=olsr" &> "${dirout}grid_400_0_${aa}_${gg}_${rr}.txt"
      fi
    done
  done
done
